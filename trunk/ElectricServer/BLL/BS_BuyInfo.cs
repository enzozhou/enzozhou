...
namespace Electric.BLL
{
	...
	{
		private readonly Electric.DAL.BS_BuyInfo dal=new Electric.DAL.BS_BuyInfo();
		public BS_BuyInfo()
		{}
		#region  Method
		...
		/// 得到一个对象实体，从缓存中
		/// </summary>
		public Electric.Model.BS_BuyInfo GetModelByCache(int ID)
		{
			
			string CacheKey = "BS_BuyInfoModel-" + ID;
			...
			return (Electric.Model.BS_BuyInfo)objModel;
		}
		...
		/// 获得数据列表
		/// </summary>
		public List<Electric.Model.BS_BuyInfo> GetModelList(string strWhere)
		{
		...
		/// 获得数据列表
		/// </summary>
		public List<Electric.Model.BS_BuyInfo> DataTableToList(DataTable dt)
		{
			List<Electric.Model.BS_BuyInfo> modelList = new List<Electric.Model.BS_BuyInfo>();
			int rowsCount = dt.Rows.Count;
			if (rowsCount > 0)
			{
				Electric.Model.BS_BuyInfo model;
				for (int n = 0; n < rowsCount; n++)
				{
					model = new Electric.Model.BS_BuyInfo();
					...
					if(dt.Rows[n]["TotalWeight"]!=null && dt.Rows[n]["TotalWeight"].ToString()!="")
					{
						model.TotalWeight=decimal.Parse(dt.Rows[n]["TotalWeight"].ToString());
					}
