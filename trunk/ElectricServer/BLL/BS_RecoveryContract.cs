...
namespace Electric.BLL
{
	...
	{
		private readonly Electric.DAL.BS_RecoveryContract dal=new Electric.DAL.BS_RecoveryContract();
		...
		/// 得到一个对象实体，从缓存中
		/// </summary>
		public Electric.Model.BS_RecoveryContract GetModelByCache(int ID)
		{
			
			string CacheKey = "BS_RecoveryContractModel-" + ID;
			...
			return (Electric.Model.BS_RecoveryContract)objModel;
		}
		...
		/// 获得数据列表
		/// </summary>
		public List<Electric.Model.BS_RecoveryContract> GetModelList(string strWhere)
		{
		...
		/// 获得数据列表
		/// </summary>
		public List<Electric.Model.BS_RecoveryContract> DataTableToList(DataTable dt)
		{
			List<Electric.Model.BS_RecoveryContract> modelList = new List<Electric.Model.BS_RecoveryContract>();
			int rowsCount = dt.Rows.Count;
			if (rowsCount > 0)
			{
				Electric.Model.BS_RecoveryContract model;
				for (int n = 0; n < rowsCount; n++)
				{
					model = new Electric.Model.BS_RecoveryContract();
					...
					if(dt.Rows[n]["DeliveryTerm"]!=null && dt.Rows[n]["DeliveryTerm"].ToString()!="")
					{
						model.DeliveryTerm=int.Parse(dt.Rows[n]["DeliveryTerm"].ToString());
					}
					...
					if(dt.Rows[n]["SPP"]!=null && dt.Rows[n]["SPP"].ToString()!="")
					{
						model.SPP=int.Parse(dt.Rows[n]["SPP"].ToString());
					}
