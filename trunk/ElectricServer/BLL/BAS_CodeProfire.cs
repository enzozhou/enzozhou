...
namespace Electric.BLL
{
	...
	{
		private readonly Electric.DAL.BAS_CodeProfire dal=new Electric.DAL.BAS_CodeProfire();
		...
		{
			return dal.Exists(SelectCode,ID);
		}
		...
		public bool Delete(string SelectCode,int ID)
		{
			...
			return dal.Delete(SelectCode,ID);
		}
		/// <summary>
		/// 删除一条数据
		/// </summary>
		...
		/// 得到一个对象实体，从缓存中
		/// </summary>
		public Electric.Model.BAS_CodeProfire GetModelByCache(int ID)
		{
			
			string CacheKey = "BAS_CodeProfireModel-" + ID;
			...
			return (Electric.Model.BAS_CodeProfire)objModel;
		}
		...
		/// 获得数据列表
		/// </summary>
		public List<Electric.Model.BAS_CodeProfire> GetModelList(string strWhere)
		{
		...
		/// 获得数据列表
		/// </summary>
		public List<Electric.Model.BAS_CodeProfire> DataTableToList(DataTable dt)
		{
			List<Electric.Model.BAS_CodeProfire> modelList = new List<Electric.Model.BAS_CodeProfire>();
			int rowsCount = dt.Rows.Count;
			if (rowsCount > 0)
			{
				Electric.Model.BAS_CodeProfire model;
				for (int n = 0; n < rowsCount; n++)
				{
					model = new Electric.Model.BAS_CodeProfire();
					...
					if(dt.Rows[n]["CodeType"]!=null && dt.Rows[n]["CodeType"].ToString()!="")
					{
