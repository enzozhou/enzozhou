...
namespace Electric.BLL
{
	...
	{
		private readonly Electric.DAL.BAS_Partner dal=new Electric.DAL.BAS_Partner();
		public BAS_Partner()
		{}
		#region  Method
		...
		/// 得到一个对象实体，从缓存中
		/// </summary>
		public Electric.Model.BAS_Partner GetModelByCache(int ID)
		{
			
			string CacheKey = "BAS_PartnerModel-" + ID;
			...
			return (Electric.Model.BAS_Partner)objModel;
		}
		...
		/// 获得数据列表
		/// </summary>
		public List<Electric.Model.BAS_Partner> GetModelList(string strWhere)
		{
		...
		/// 获得数据列表
		/// </summary>
		public List<Electric.Model.BAS_Partner> DataTableToList(DataTable dt)
		{
			List<Electric.Model.BAS_Partner> modelList = new List<Electric.Model.BAS_Partner>();
			int rowsCount = dt.Rows.Count;
			if (rowsCount > 0)
			{
				Electric.Model.BAS_Partner model;
				for (int n = 0; n < rowsCount; n++)
				{
					model = new Electric.Model.BAS_Partner();
