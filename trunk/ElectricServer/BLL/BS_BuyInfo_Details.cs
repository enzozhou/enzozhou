...
namespace Electric.BLL
{
	...
	{
		private readonly Electric.DAL.BS_BuyInfo_Details dal=new Electric.DAL.BS_BuyInfo_Details();
		...
		/// 得到一个对象实体，从缓存中
		/// </summary>
		public Electric.Model.BS_BuyInfo_Details GetModelByCache(int ID)
		{
			
			string CacheKey = "BS_BuyInfo_DetailsModel-" + ID;
			...
			return (Electric.Model.BS_BuyInfo_Details)objModel;
		}
		...
		/// 获得数据列表
		/// </summary>
		public List<Electric.Model.BS_BuyInfo_Details> GetModelList(string strWhere)
		{
		...
		/// 获得数据列表
		/// </summary>
		public List<Electric.Model.BS_BuyInfo_Details> DataTableToList(DataTable dt)
		{
			List<Electric.Model.BS_BuyInfo_Details> modelList = new List<Electric.Model.BS_BuyInfo_Details>();
			int rowsCount = dt.Rows.Count;
			if (rowsCount > 0)
			{
				Electric.Model.BS_BuyInfo_Details model;
				for (int n = 0; n < rowsCount; n++)
				{
					model = new Electric.Model.BS_BuyInfo_Details();
					...
					if(dt.Rows[n]["InvoiceDate"]!=null && dt.Rows[n]["InvoiceDate"].ToString()!="")
					{
					...
					if(dt.Rows[n]["NewRating"]!=null && dt.Rows[n]["NewRating"].ToString()!="")
					{
						model.NewRating=decimal.Parse(dt.Rows[n]["NewRating"].ToString());
					}
					if(dt.Rows[n]["NewVoltage"]!=null && dt.Rows[n]["NewVoltage"].ToString()!="")
					{
						model.NewVoltage=decimal.Parse(dt.Rows[n]["NewVoltage"].ToString());
					}
					if(dt.Rows[n]["NewSpeed"]!=null && dt.Rows[n]["NewSpeed"].ToString()!="")
					{
						model.NewSpeed=decimal.Parse(dt.Rows[n]["NewSpeed"].ToString());
					}
					if(dt.Rows[n]["NewProtectionLev"]!=null && dt.Rows[n]["NewProtectionLev"].ToString()!="")
					{
					model.NewProtectionLev=dt.Rows[n]["NewProtectionLev"].ToString();
					}
					if(dt.Rows[n]["NewMC"]!=null && dt.Rows[n]["NewMC"].ToString()!="")
					{
					model.NewMC=dt.Rows[n]["NewMC"].ToString();
					}
					if(dt.Rows[n]["NewWeight"]!=null && dt.Rows[n]["NewWeight"].ToString()!="")
					{
						model.NewWeight=decimal.Parse(dt.Rows[n]["NewWeight"].ToString());
					}
