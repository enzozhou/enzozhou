...
namespace Electric
{
    public partial class frm_department_list : Form
    {
        public frm_department_list()
        {
            InitializeComponent();
        }
        ...
        {
            global.FormDialog(new frm_department(), "部门信息");
        }
                ...
                if (item.Selected)
                {
                    if (int.TryParse(item.Cells["ID"].Value.ToString(), out _id))
                    {
                        ...
                        if (_id > 0)
                        {
                            ...
                            {
                                new Electric.BLL.BAS_Department().Delete(_id);
        ...
        private void btnUP_Click(object sender, EventArgs e)
        {
            if (_pageStartIndex >= _pagesize)
            {
                ...
                DataSet _ds = new BLL.BAS_Department().GetListByPage(_sqlWhere, "ID", _pageStartIndex, _pageEndIndex);
                dgv.DataSource = _ds;
                dgv.DataMember = "ds";
                setGridTitle();
            }
