...
namespace Electric.DAL
{
	...
	public partial class V_BAS_PPC
	{
		...
		public int GetMaxId()
		{
		return DbHelperSQL.GetMaxID("ID", "V_BAS_PPC"); 
		}
		...
		public bool Exists(int ID)
		{
			...
			strSql.Append("select count(1) from V_BAS_PPC");
		...
		{
			StringBuilder strSql=new StringBuilder();
			strSql.Append("insert into V_BAS_PPC(");
		...
		{
			StringBuilder strSql=new StringBuilder();
			strSql.Append("update V_BAS_PPC set ");
		...
		public bool Delete(int ID)
		{
			...
			StringBuilder strSql=new StringBuilder();
			strSql.Append("delete from V_BAS_PPC ");
			strSql.Append(" where ID=@ID");
			SqlParameter[] parameters = {
					new SqlParameter("@ID", SqlDbType.Int,4)
			};
		...
		public Electric.Model.V_BAS_PPC GetModel(int ID)
		{
			...
			Electric.Model.V_BAS_PPC model=new Electric.Model.V_BAS_PPC();
			...
			if(ds.Tables[0].Rows.Count>0)
			{
                ...
                {
                    model.BankName = ds.Tables[0].Rows[0]["ContractNo"].ToString();
                }
                if (ds.Tables[0].Rows[0]["Membership"] != null && ds.Tables[0].Rows[0]["Membership"].ToString() != "")
                {
                    model.BankName = ds.Tables[0].Rows[0]["Membership"].ToString();
                }
				return model;
			}
			else
		...
		public DataSet GetListByPage(string strWhere, string orderby, int startIndex, int endIndex)
		{
			...
			strSql.Append(")AS Row, T.*  from V_BAS_PPC T ");
