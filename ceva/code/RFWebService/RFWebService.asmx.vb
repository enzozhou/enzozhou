...
Public Function get_svr_datetime() As String
        Dim str As String = Now().ToString("u")
        Return str
    End Function
...
#Region "业务处理函数:"
    ...
    ''' <summary>
    ''' 提取当前物品的任务单据列表-01:无分配的情况
    ''' </summary>
    ''' <param name="as_sn"></param>
    ''' <param name="as_sku_no"></param>
    ''' <param name="as_assigned_opt"></param>
    ''' <param name="as_sErr"></param>
    ''' <returns></returns>
    ''' <remarks></remarks>
    <WebMethod(Description:="执行指定存储过程 sp_hht_getTaskList_div ,返回数据集")> _
    Public Function wf_sp_hht_getTaskList(ByVal as_sn As String, ByVal as_sku_no As String, ByVal as_assigned_opt As String, ByRef as_sErr As String) As DataSet
        '调用..
        Try
            Dim ds As DataSet = RunProcedure_DS(as_sn, "sp_hht_getTaskList_div", "@sku_no|varchar|" + as_sku_no + "|input;@assigned_opt|varchar|" + as_assigned_opt + "|input;@sErr|varchar||output", as_sErr)
            If as_sErr <> "" Then
                Return Nothing
            End If
            ' --    业务单据分配过程::
            ' task_id,bch_no, dn_no, bin_code, qty, rqty, task_id ,sku_no,assigned_opt, opt_by
            'TASK1105030072	CEVA1105030002	9204506598	A-A-05	12.000	P02229172	0.000	110
            'TASK1105030072	CEVA1105030002	9204506585	A-A-08	1.000	P02229172	0.000	110
            '-- 110
            '得到 表1记录行数
            Dim rlt_CNT As Integer = ds.Tables(0).Rows.Count

            '得到用户数量
            Dim user_CNT As Integer = ds.Tables(1).Rows.Count

            '得到当前用户是活动用户中的第几个:
            Dim u_index As Integer = 1 '默认为只有一个人.

            For ii As Integer = 0 To ds.Tables(1).Rows.Count - 1
                If as_assigned_opt = ds.Tables(1).Rows(ii)("opt_by").ToString() Then
                    u_index = ii + 1
                    Exit For
                End If
            Next

            'as_sErr = "rlt_CNT=" + u_index.ToString() + " +  "

            Dim next_rowIndex As Integer = 0 '下一个取值点
            ' as_sErr = ds.Tables(1).Rows.Count.ToString() + "  " + u_index.ToString()
            '按照用户序号,该用户属于第 u_index 位置,则记录中 为此值倍数的记录,返回给他
            Dim dt As DataTable = ds.Tables(0).Clone()
            For jj As Integer = 0 To ds.Tables(0).Rows.Count - 1
                'If (jj + 1) Mod u_index = 0 Then
                If jj + 1 = next_rowIndex * user_CNT + u_index Then
                    'as_sErr = as_sErr + (jj + 1).ToString() + ","
                    'task_id,bch_no, dn_no, bin_code, qty, rqty, ,sku_no,assigned_opt
                    'task_id,bch_no, dn_no, bin_code, qty, rqty, ,sku_no,assigned_opt, opt_by
                    Dim dr As DataRow = ds.Tables(0).NewRow()
                    dr("task_id") = ds.Tables(0).Rows(jj)("task_id")
                    dr("bch_no") = ds.Tables(0).Rows(jj)("bch_no")
                    dr("dn_no") = ds.Tables(0).Rows(jj)("dn_no")
                    dr("bin_code") = ds.Tables(0).Rows(jj)("bin_code")
                    dr("qty") = ds.Tables(0).Rows(jj)("qty")
                    dr("rqty") = ds.Tables(0).Rows(jj)("rqty")
                    dr("sku_no") = ds.Tables(0).Rows(jj)("sku_no")
                    dr("assigned_opt") = ds.Tables(0).Rows(jj)("assigned_opt")
                    dt.Rows.Add(dr.ItemArray)
                    next_rowIndex = next_rowIndex + 1
                End If
            Next
            Dim dsRtn As DataSet = New DataSet()
            dsRtn.Tables.Add(dt)
            'as_assigned_opt
            '按照当前用户返回所属的记录
            Return dsRtn
