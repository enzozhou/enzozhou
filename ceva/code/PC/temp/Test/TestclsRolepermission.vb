...
'******************************************************************************
'*
'* Name:        clsRolepermissionTest
'*
'* Description: NUnit tests for clsRolepermission
'*
'******************************************************************************

<TestFixture()> Public Class clsRolepermissionTest

    '// Business Objects we are testing
    Dim clsRolepermissionCollection As clsRolepermissions

    '// Data that already exists in the database (before the tests run)
    Dim ExistingObject As clsRolepermission
	
    '// Data thet will be added by the tests and deleted by the tests
    Dim NewObject As clsRolepermission
...
#Region "Setup, Hardcoded test Data"
    ...
    <SetUp()> Public Sub SetUp()
        ...
        ExistingObject = clsRolepermission.NewclsRolepermission 
        ...
        NewObject = clsRolepermission.NewclsRolepermission
        'NewObject.remark= ?
        'NewObject.opt_by= ?
        'NewObject.opt_dtime= ?
    ...
    Private Sub ModifyObject(ByVal o As clsRolepermission)

    End Sub
    
    'TODO: add some lines to undo the changes to the existing object by "ModifyObject"
    Private Sub UnModifyObject(ByVal o As clsRolepermission)
...
#Region "Business Object Validation Tests"
    ...
    <Test()> Public Sub TestCollectionCountAfterLoad()
        clsRolepermissionCollection = clsRolepermissions.Fetch(New CXFilters(), 0)
        
        If clsRolepermissionCollection.Count > 0 andalso ExistingObject.IsNew Then
        	ExistingObject = clsRolepermissionCollection.Item(0)
        End If
        
        Assert.AreEqual(RecordCount, clsRolepermissionCollection.Count, "Collection should contain '" & RecordCount.ToString() & "' records. It contains '" & clsRolepermissionCollection.Count.ToString & "' records.")
    End Sub
    
    <Test()> Public Sub TestCollectionCountAfterFind()
        clsRolepermissionCollection = clsRolepermissions.Fetch(FindFilters, 0)
...
#Region "Test Properties"
    ...
    <Test()> Public Sub TestPropertiesAfterLoad()
    	If not ExistingObject.IsNew Then
        	Dim o As clsRolepermission = clsRolepermission.Fetch(ExistingObject.role_code,ExistingObject.right_no, 0)
...
#Region "Test Standard Methods"
    ...
    <Test()> Public Sub TestClone()
        Dim o As clsRolepermission = CType(ExistingObject.Clone(), clsRolepermission)
		...
		o = CType(NewObject.Clone(), clsRolepermission)
...
#Region "Tests For IsNew, IsDirty, IsDeleted Flag"
    ...
    <Test()> Public Sub TestFlagAfterConstructor()
        Dim o As clsRolepermission = clsRolepermission.NewclsRolepermission 
    ...
    <Test()> Public Sub TestFlagAfterInsertSave()
        Dim o As clsRolepermission = ctype(NewObject.Clone(), clsRolepermission)
        If o.IsSavable Then
	        o.Save()
        	Assert.AreEqual(False, o.IsNew, "IsNew Flag should be 'FALSE' after constructor is called.")
        	Assert.AreEqual(False, o.IsDirty, "IsDirty Flag should be 'FALSE' after constructor is called.")
