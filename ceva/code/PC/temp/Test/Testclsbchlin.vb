...
'* Name:        clsbchlinTest
'*
'* Description: NUnit tests for clsbchlin
'*
'******************************************************************************

<TestFixture()> Public Class clsbchlinTest

    '// Business Objects we are testing
    Dim clsbchlinCollection As clsbchlins

    '// Data that already exists in the database (before the tests run)
    Dim ExistingObject As clsbchlin
	
    '// Data thet will be added by the tests and deleted by the tests
    Dim NewObject As clsbchlin
...
#Region "Setup, Hardcoded test Data"
    ...
    <SetUp()> Public Sub SetUp()
        ...
        ExistingObject = clsbchlin.Newclsbchlin 
        ...
        NewObject = clsbchlin.Newclsbchlin
    ...
    Private Sub ModifyObject(ByVal o As clsbchlin)

    End Sub
    
    'TODO: add some lines to undo the changes to the existing object by "ModifyObject"
    Private Sub UnModifyObject(ByVal o As clsbchlin)
...
#Region "Business Object Validation Tests"
    ...
    <Test()> Public Sub TestCollectionCountAfterLoad()
        clsbchlinCollection = clsbchlins.Fetch(New CXFilters(), 0)
        
        If clsbchlinCollection.Count > 0 andalso ExistingObject.IsNew Then
        	ExistingObject = clsbchlinCollection.Item(0)
        End If
        
        Assert.AreEqual(RecordCount, clsbchlinCollection.Count, "Collection should contain '" & RecordCount.ToString() & "' records. It contains '" & clsbchlinCollection.Count.ToString & "' records.")
    End Sub
    
    <Test()> Public Sub TestCollectionCountAfterFind()
        clsbchlinCollection = clsbchlins.Fetch(FindFilters, 0)
...
#Region "Test Properties"
    ...
    <Test()> Public Sub TestPropertiesAfterLoad()
    	If not ExistingObject.IsNew Then
        	Dim o As clsbchlin = clsbchlin.Fetch(ExistingObject.dc_code,ExistingObject.bch_no,ExistingObject.dn_no, 0)
...
#Region "Test Standard Methods"
    ...
    <Test()> Public Sub TestClone()
        Dim o As clsbchlin = CType(ExistingObject.Clone(), clsbchlin)
		...
		o = CType(NewObject.Clone(), clsbchlin)
...
#Region "Tests For IsNew, IsDirty, IsDeleted Flag"
    ...
    <Test()> Public Sub TestFlagAfterConstructor()
        Dim o As clsbchlin = clsbchlin.Newclsbchlin 
    ...
    <Test()> Public Sub TestFlagAfterInsertSave()
        Dim o As clsbchlin = ctype(NewObject.Clone(), clsbchlin)
        If o.IsSavable Then
	        o.Save()
        	Assert.AreEqual(False, o.IsNew, "IsNew Flag should be 'FALSE' after constructor is called.")
        	Assert.AreEqual(False, o.IsDirty, "IsDirty Flag should be 'FALSE' after constructor is called.")
